...
MISSING=()
for var in AZURE_OPENAI_ENDPOINT AZURE_OPENAI_API_KEY AZURE_OPENAI_DEPLOYMENT AZURE_SPEECH_KEY AZURE_SPEECH_REGION AUTH_EMAIL AUTH_PASSWORD AUTH_SECRET; do
    if [[ -z "${!var:-}" ]]; then
        MISSING+=("$var")
    fi
done
